...
<div class="px-4 sm:px-6 lg:px-8 lg:py-14 mx-auto">
    <div class="max-w-2xl mx-auto text-center mb-10 lg:mb-14">
        ...
        @if ($healthCare->type == 'Doctor')
            <p class="mt-1 text-gray-600">Meet our top doctors, leading the way in medical excellence and patient care.</p>
        @elseif ($healthCare->type == 'Hospital')
            <p class="mt-1 text-gray-600">Discover our top hospitals, setting the standard in medical excellence and patient care.</p>
        @elseif ($healthCare->type == 'Pharmacy')
            <p class="mt-1 text-gray-600">Explore our leading pharmacies, committed to medical excellence and patient care.</p>
        @elseif ($healthCare->type == 'Medical')
            <p class="mt-1 text-gray-600">Learn more about our top medical services, pioneering excellence in patient care.</p>
        @endif
    </div>
                   ...
                   href="{{ route('healthcare.detailPage', $healthCareList) }}">
                    <div class="aspect-w-16 aspect-h-9 overflow-hidden rounded-xl">
                        <img class="w-full h-full object-cover rounded-xl transition-transform duration-300 transform group-hover:scale-105"
                             src="{{ count($healthCareList->files) > 0 ? $healthCareList->files?->first()->file_url : '' }}" alt="Blog Image">
                    </div>
                    <h3 class="mt-4 text-lg sm:text-xl text-gray-700 hover:text-neutral-800">
                        {{ $healthCareList->name }}
                    </h3>
                    <button class="mt-3 bg-neutral-700 hover:bg-neutral-800 inline-flex items-center gap-x-1 text-sm font-semibold text-white rounded-lg px-3 py-2">
                        Learn more
