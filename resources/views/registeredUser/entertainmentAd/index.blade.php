...
@section('content')
    <div class="content px-5 md:px-7 col-span-3 mt-8 md:mt-0 font-manrope min-h-screen">
        <h1 class="font-semibold text-3xl">Entertainment List</h1>
        <div class="border-b border-gray-200 dark:border-neutral-700">
            <nav class="flex gap-x-1 bg-neutral-200 hover:bg-neutral-400 mt-3" aria-label="Tabs" role="tablist" aria-orientation="horizontal">
                @foreach ($mainCategories as $mainCategory)
                    @foreach ($mainCategory->entertainmentCategories as $index => $entertainmentCategory)
                        <button type="button"
                            ...
                            id="tab-{{ $entertainmentCategory->id }}"
                            data-hs-tab="#tab-content-{{ $entertainmentCategory->id }}"
                            aria-controls="tab-content-{{ $entertainmentCategory->id }}"
                            role="tab"
        ...
        <div class="mt-3">
            @foreach ($mainCategories as $mainCategory)
                @foreach ($mainCategory->entertainmentCategories as $index => $entertainmentCategory)
                    <div id="tab-content-{{ $entertainmentCategory->id }}" class="tab-content" role="tabpanel"
                        aria-labelledby="tab-{{ $entertainmentCategory->id }}" style="display: {{ $index == 0 ? 'block' : 'none' }};">
                                ...
                                <div class="p-1.5 min-w-full inline-block align-middle">
                                    <div class="overflow-hidden">
                                        <x-frontend.forms.table-component
                                            ...
                                            :data="$entertainmentCategory->entertainmentList">
                                            @forelse ($entertainmentCategory->entertainmentLists as $entertainmentList)
                                                <tr>
                                                    <td
                                                        class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-neutral-200">
                                                        {{ $entertainmentList->reference_no }}
                                                    </td>
                                                    <td
                                                        class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-neutral-200">
                                                        {{ Str::words($entertainmentList->name, 5) }}
                                                    </td>
                                                    <td
                                                    ...
                                                    <td
                                                        class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-neutral-200">
                                                        {{ $entertainmentList->contact_number }}
                                                    </td>

                                                    <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                                                        <a type="button"
                                                           href="{{ route('registeredUser.entertainmentList.edit', $entertainmentList) }}"
                                                        ...
                                                        <form
                                                            action="{{ route('registeredUser.entertainmentList.destroy', $entertainmentList) }}"
